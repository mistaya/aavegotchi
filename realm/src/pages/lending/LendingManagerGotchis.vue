      ...
      >
        ...
        <template #rows>
          ...
          >
            <td>
              <a
                :href="getGotchiUrl({ network: selectedNetwork, gotchiId: row.gotchi.id, ownerAddress: row.listing?.originalOwner || row.listing?.lender || address })"
                rel="noopener"
                target="_blank"
              >
                {{ row.gotchi.name }}
...
export default {
  ...
  setup (props) {
    ...
    const fetchOwnedGotchis = function () {
      ...
      const OWNER_SUBGRAPH_URL = apis[selectedNetwork.value].CORE_SUBGRAPH
